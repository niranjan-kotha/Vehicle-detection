...
def slide_windows(img):
    ...
    for (xy_window, y_start_stop,overlap) in zip(WINDOW_SIZES, WINDOW_Y_START_STOP,WINDOW_OVERLAP):
        ...
        nx_pix_per_step = np.int(xy_window[0] * (1. - overlap))
        ny_pix_per_step = np.int(xy_window[1] * (1. - overlap))
...
def pipeline(image):
    ...
    heatmap_image = np.zeros((image.shape[0], image.shape[1]), np.uint8)
    ...
    heatmap_image = apply_threshold(heatmap_image, 8) #4
    ...
    print(heatmap_image[heatmap_image>7])

    final_image = draw_labeled_bboxes(image, heatmap_image)    
